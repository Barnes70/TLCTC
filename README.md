...
**Version 2.6** · CC BY 4.0 · [![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20633176.svg)](https://doi.org/10.5281/zenodo.20633176) · [tlctc.net](https://www.tlctc.net) · [Core Paper (citable)](documentation/tlctc-v2.6-core.md) · [White Paper](https://www.tlctc.net/tlctc-v2.0-whitepaper.html)
...
TLCTC v2.6 is the current citable version: the v2.3 consolidation freeze of the finalized v2.1 framework, plus the v2.4 interaction-model clarification, the v2.5 disambiguation and scope rules, and the v2.6 answers to an external review — #10 requires subversion, R-SPECIFIC replaces three precedence rules, successes-only classification (see [`tlctc-traceability.md`](documentation/tlctc-traceability.md) and the core paper's Appendix A). The documents form a deliberate hierarchy:

| Document | Role |
|---|---|
| [**Core Paper**](documentation/tlctc-v2.6-core.md) ([PDF](documentation/tlctc-v2.6-core.pdf)) | **The canonical definition** of the framework: derivation, 10 clusters, 10 axioms, 17 classification rules, attack-path notation, glossary, references. Citable (see How to Cite). |
| [**Application Paper**](documentation/tlctc-v2.6-application.md) ([PDF](documentation/tlctc-v2.6-application.pdf), [DOI](https://doi.org/10.5281/zenodo.22697636)) | Companion for putting the taxonomy to work: classification procedure, worked examples, NIST CSF mapping, controls, KRI/KCI/KPI. Takes the core as given. Separately citable (DOI). |
...
Machine-readable twin of the core: [`json-schemas/layer-1/tlctc-framework.v2.6.json`](json-schemas/layer-1/tlctc-framework.v2.6.json). The v2.3, v2.4 and v2.5 dictionaries are retained unchanged as frozen records for classifications made under those versions.
...
Adversarial techniques against AI-enabled systems classify by the same ten clusters. The [`mappings/mitre-atlas/`](mappings/mitre-atlas/) directory maps **every MITRE ATLAS technique and sub-technique** (all 208 of release v2026.09, plus the three ids that release retired but the STIX distribution still carries) to TLCTC in the same record shape and notation as the ATT&CK mapping: prompt injection, jailbreaks, RAG and context poisoning and agent tool abuse are #1 (the model or agent is the capability vector, not the actor), supply-chain techniques are #10 at the Trust Acceptance Event, attacker-side staging is N/A, and model, data and context degradation are Data Risk Events rather than clusters. Techniques that mirror ATT&CK carry the ATT&CK mapping over verbatim. The Attack Flow classifier uses it for actions that carry ATLAS ids.

See the [mapping README](mappings/mitre-atlas/README.md) for the doctrine the rows follow and the [decision tree](mappings/mitre-atlas/decision-tree.md) for the AI-specific questions placed in front of the ATT&CK tree.

### VERIS → TLCTC

Incident records carry the cause their actions imply. The [`mappings/veris/`](mappings/veris/) directory maps **every VERIS 1.4.1 action variety, vector, result and attribute value** (337 entries) to TLCTC: the cluster whose generic vulnerability the action exploits, the rule that decides where VERIS cannot (`Exploit vuln` is #2 or #3 by R-ROLE), the companion step the rules require but VERIS does not record (malware needs its delivery, credential use its acquisition), the cause-side partition row for errors, environmental events and insider misuse inside a genuine entitlement (Abuse of Rights, R-SCOPE) that are not threats, and the Data Risk Event code for outcomes (`Obscuration` is `Ac`, `Loss` is `Av`). A generated CSV in the layout of the VERIS repository's own ATT&CK crosswalk is the upstream deliverable; a stdlib Python classifier applies the mapping to VERIS/VCDB records; and a reproducible study runs it over the 10,047 records of the VERIS Community Database, with the write-up in [`documentation/tlctc-veris-vcdb-study.md`](documentation/tlctc-veris-vcdb-study.md).
...
|---|---|---|---|---|
| **Layer 1 — Framework Definition** | Immutable "dictionary": clusters, axioms, rule IDs, topology types | `tlctc-framework.schema.json` / `tlctc-framework.v2.5.json` | Universal | Rarely (framework evolution) |
...
│   ├── mitre-atlas/                          # MITRE ATLAS → TLCTC (211 techniques, AI-enabled systems)
│   │   ├── tlctc-atlas.json                  # Technique mappings with rationale
│   │   └── decision-tree.md                  # AI-specific questions before the ATT&CK tree
...
│   │   ├── tlctc-framework.v2.5.json         # V2.5.1 frozen record (retained for 2.5.x classifications)
│   │   └── tlctc-framework.v2.6.json         # V2.6 canonical dictionary: clusters, axioms, 17 rules, DRE tree, cause-side partition, SRE types, FEC boundary
...
├── documentation/
│   ├── tlctc-v2.6-core.md                    # ⭐ Core paper (v2.6, citable)
...
│   ├── tlctc-v2.5-core.md                    # Core paper v2.5.1 (previous deposit; frozen)
│   ├── tlctc-v2.5-core.pdf                   # PDF of the v2.5.1 core paper
...
│   ├── tlctc-traceability.md            # consolidation & version traceability (v2.1 → v2.6)
...
│   ├── tlctc-plus-ncsc-proposal.md           # TLCTC+ national reporting extension — policy proposal (v0.8)
│   ├── tlctc-plus-specification.md           # TLCTC+ implementation specification (v0.8) — grammar, conformance, catalogues
...
1. **Read the core paper** — [`documentation/tlctc-v2.6-core.md`](documentation/tlctc-v2.6-core.md) is the canonical definition: derivation, clusters, axioms, rules, and notation in one self-contained paper.
...
8. **Classify AI-system threats** — See [`mappings/mitre-atlas/`](mappings/mitre-atlas/) for every MITRE ATLAS technique placed in the ten clusters, with the AI-specific decision questions.
9. **Track active exploitation** — See [`mappings/cisa-kev/`](mappings/cisa-kev/) for a TLCTC-cluster view of the CISA Known Exploited Vulnerabilities catalog (1,568 CVEs, weekly-refreshable, deterministically derived).
10. **Audit your detection coverage** — See [`mappings/sigma/`](mappings/sigma/) for a per-rule TLCTC cluster derivation of 3,132 SigmaHQ detection rules — cross-walk your SOC ruleset against the strategic cluster model.
11. **Read incident records by cause** — See [`mappings/veris/`](mappings/veris/) to classify VERIS/VCDB incident records into clusters, and the [VCDB study](documentation/tlctc-veris-vcdb-study.md) for what a cause axis recovers from 10,047 records and what only sequencing can.
12. **Bring cause into Attack Flow** — See [`integrations/attack-flow/`](integrations/attack-flow/) to classify any Attack Flow through the ATT&CK→TLCTC mapping, export a TLCTC path into the Builder, and read the [corpus study](documentation/tlctc-attack-flow-corpus-study.md).
13. **Use the glossary** — See [`glossary/`](glossary/) for precise, machine-readable definitions of all TLCTC terms and cyber security vocabulary.
...
|---|---|
| [Core Paper v2.6](documentation/tlctc-v2.6-core.md) ([PDF](documentation/tlctc-v2.6-core.pdf)) | The canonical definition of the framework |
...
> Kreinz, B. (2026). *A Cause-Oriented Cyber Threat Taxonomy: The Top Level Cyber Threat Clusters Framework* (Version 2.6) [Preprint]. Zenodo. https://doi.org/10.5281/zenodo.20633176
...
To cite the **application paper** instead — the companion on classification
procedure, governance, and cross-domain integration — use its own record:

> Kreinz, B. (2026). *Applying the Top Level Cyber Threat Clusters: Classification, Governance, and Cross-Domain Application* (Version 2.6) [Preprint]. Zenodo. https://doi.org/10.5281/zenodo.22697636

```bibtex
@misc{kreinz2026tlctcapplication,
  author    = {Kreinz, Bernhard},
  title     = {Applying the Top Level Cyber Threat Clusters: Classification,
               Governance, and Cross-Domain Application},
  year      = {2026},
  version   = {2.6},
  publisher = {Zenodo},
  doi       = {10.5281/zenodo.22697636},
  url       = {https://doi.org/10.5281/zenodo.22697636}
}
```

Cite the core paper for the taxonomy itself; cite the application paper only for
material it introduces, since it redefines no normative element.
