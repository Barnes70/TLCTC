...
- `tlctc` taxonomy: `tlctc:cluster="NN-slug"` (one per distinct cluster in the path) and exclusive `tlctc:entry-cluster="NN-slug"`; descriptions are the v2.6 dictionary strings verbatim, generated by `scripts/build-misp-taxonomy.js`.
...
- `stix/tlctc-stix-bundle.json`: the framework as a STIX 2.1 source in the shape the Builder consumes for ATLAS and F3 — ten clusters as tactics, cluster roots and published sub-clusters as techniques (generated from the v2.6 dictionary by `scripts/build-attack-flow-stix.js`).
