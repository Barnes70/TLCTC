# TLCTC XSOAR Integration — Test Cases

Five canonical scenarios. Each one verifies a specific TLCTC classification rule
that the integration must enforce. Run them in order after install.

For each test:

1. Inject a synthetic alert with the listed `attack.technique.id` value(s).
2. Confirm `incident_tlctccluster` is set to **Expected Cluster** within 5 s.
3. Confirm the `TLCTCSequence` label matches **Expected Sequence**.
4. Confirm the listed playbooks fire in order.
5. On close, confirm the emitted `incident_tlctcattackpath` validates against
   `json-schemas/layer-3/tlctc-attack-path.schema.json`.
...
both playbooks; the FEC execution (#7) must record `fec_executed: true`.
...
**Why this matters:** Credential APPLICATION is always #4 regardless of how the
credential was obtained. If a separate alert with T1003 (credential dumping)
also exists, that fires #1 separately — they are different steps.
...
flaw class in a browser would be #3. The integration must not collapse to "web
attack" or similar outcome label.
...
**Why this matters:** #10 is placed at the Trust Acceptance Event. The boundary
field MUST be populated. The RS Container's regulatory branches will fire if
DRE includes Confidentiality on PII (GDPR) or severity ≥ 3 (NIS2).
...
All five test cases must pass before the integration is promoted to production.
Failures indicate either the lookup list is stale or one of the classification
rules in `automations/TLCTCClassify.yml` is incorrectly implemented.

The Layer 3 emit step on close must produce a JSON instance that passes:

```bash
ajv validate -s json-schemas/layer-3/tlctc-attack-path.schema.json -d <emitted-path>.json
```
