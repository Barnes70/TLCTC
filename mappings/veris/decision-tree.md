...
Did an accountable grantor confer an entitlement covering THIS action?
  yes → no-cluster, partition_row = abuse_of_rights   (misuse: Unapproved hardware/software/workaround,
                                                     Net misuse, Email misuse, Illicit content)
...
"Functioned as designed" does not mean in-grant: `Privilege abuse`, `Possession abuse`,
`Knowledge abuse`, `Data mishandling`, `Snap picture` use functions the insider could reach for
a purpose outside the grant, and are #1.
...
Q2.1 Does the value describe foreign executable content running?              (R-EXEC)
       every malware.variety                     → chain #7, companion BEFORE = the enabling cluster
                 ...
                 Exploit vuln (conditional #2 | #3, the enabler of #7), Exploit misconfig (#1, the enabler),
                 Client-side attack (#3), Brute force / Pass-the-hash (#7 → #4, R-CRED),
...
Q2.2 Does the value describe authenticating as an identity that is not the presenter's own?   (R-CRED, Axiom X)
       ...
       Brute force                                                             → direct #4 (guessing is application)
...
Q2.4 Does the value describe interception, relay or a channel-control defect?  (R-MITM, R-CHANNEL)
       hacking AitM, Routing detour             → chain #5, companion BEFORE = position acquisition
...
Q2.6 Does the value describe exploiting a code flaw?                           (R-ROLE)
       ...
       its children                             → direct #2 with role_hint server
         except XSS, CSRF, URL redirector abuse → direct #3 (client-role component acts on attacker content)
...
Q2.7 Does the value describe using a designed function beyond its purpose?
       Abuse of functionality, Exploit misconfig, Hijack, Disable controls, Profile host,
       Scan network, Prompt injection, and the out-of-grant misuse varieties   → direct #1
