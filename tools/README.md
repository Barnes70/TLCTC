...
The Path Atlas is the one tool that is not a single file: it loads `data/path-corpus.js` (the bundled records, generated by `npm run build-path-corpus`) and `lib/path-stats.js` (the counting rules, shared with Node tests in `scripts/test-path-stats.js`) as relative `<script src>` files, so copy the `data/` and `lib/` folders along with it. Both load from disk without a server.
