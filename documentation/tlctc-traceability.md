...
## v2.5 — R-CRED Self-Issued-Identity Clarification (2026-08-14)

A single normative clarification to **R-CRED** (self-issued identity), applied to the **in-progress v2.5 editing version** — v2.5 remains open in the repository until declared finished, at which point one Zenodo v2.5 deposit is minted; this is v2.5 content, not a post-release erratum. `tlctc_version` stays **2.5** and `schema_version` is unchanged at 2.0.0 — no schema-shape change. No cluster identity, ID, definition, attacker's view, generic vulnerability, or topology changed; the axiom set and rule count (**18**) are unchanged.
...
**Not fully classification-preserving.** Unlike the v2.4 clarification, this change can alter a decision: records that classified *use* of a self-enrolled account as #4 SHOULD be re-checked against the proviso. Fictitious/pseudonymous self-registration is #1; enrolment completed as an existing identity is #1 → #4. It joins the v2.5 delta (with R-CHANNEL, R-SUBSTRATE, and the #2/#3 retightening) that makes v2.5 non-classification-preserving relative to v2.4.
...
**Harmonization.** Applied across `tlctc-framework.v2.5.json` (R-CRED statement + notes, metadata notes), `tlctc-v2.5-core.md` (declaration, §6 R-CRED, §4 #1/#4 boundary tests), `tlctc-glossary.md` (R-CRED entry, quick-reference table, Credential Application entry), `glossary/tlctc-glossary.json` (R-CRED entry), the `tlctc-classify` skill (R-CRED boundary tests, rule section, Kerberos LOTL worked example), and `tlctc-ti-sharing-profile.md` (§3.1 field + consistency check). The generated `okf/` bundle, HTML docs, and PDFs are rebuilt from these sources.

## Erratum — v2.5.1 (2026-09-10)

A post-deposit erratum to v2.5 (first v2.5 Zenodo deposit: 2026-09-06, version DOI 10.5281/zenodo.22549796). Two normative corrections and three consistency repairs. The framework specification remains **v2.5**; `tlctc_version` in the dictionary is unchanged, as in the 2.3.x errata. **Classification-preserving:** no step classified under v2.5 changes cluster.

| Field | Before | After |
|---|---|---|
| `#8` definition | "Unauthorized physical interaction with or interference to hardware, **facilities**, media, interfaces, or signals—via direct contact or exploitation of physical phenomena/emanations." | "Unauthorized physical interaction with or interference to hardware, media, interfaces, or signals—via direct contact or exploitation of physical phenomena/emanations." |
| `system_risk_event` | two co-equal types at one altitude: `compromise` and `failure` | one defined type, `compromise`; `failure` retained verbatim as the standing example in a new `other_system_altitude_events` block (schema `types` cardinality 2 → 1) |
| `system_risk_event.definition` | silent on cardinality per step | states that every cluster step records an SRE; a path of *n* cluster steps records *n*, each admitting a DRE, a chained SRE, or both |

**Justification (axiom-level, per the immutability rule).** *#8:* the cluster's subject is hardware and OT reached physically; a facility is the envelope through which hardware is reached, not the thing whose behaviour leaves its owner's control. Naming it in the definition invited reading building intrusion with no hardware interaction as a cyber threat step. Attacker's view, generic vulnerability ("physical accessibility of infrastructure"), ID, topology and boundary tests are unchanged. *SRE:* the 2026-09-05 widening named System Failure as a second type of the SRE. The framework's figures draw System Compromise alone, and the ten clusters converge on Compromise and on nothing else, so the text had elevated Failure above what the model shows and presented the clusters' pivot as one of two peers. Re-centering restores the figure/text agreement the widening was meant to achieve. *Per-step rule:* the thought experiment derives each cluster as a distinct generic vulnerability by which control departs from the owner; identifying a cluster therefore asserts that departure, so each classified step is a compromise and records its own SRE. Core §3.4 had already implied this by naming a data-reading SQL injection as a compromise with a simultaneous consequence; it is now explicit in the dictionary. Corollary made explicit in §3.4: a compromise is judged by the behaviour the system was made to perform, never by what the attacker obtained — the latter is the DRE, and letting it settle a cause-side question would breach Axiom III.
