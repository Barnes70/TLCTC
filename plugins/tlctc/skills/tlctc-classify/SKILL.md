...
- FEC is content executed by a **general-purpose execution engine** — an OS loader, shell, script interpreter, macro engine, browser JavaScript engine, module or plugin loader, or general-purpose language runtime
- Domain-specific expression languages (SQL, LDAP, XPath, GraphQL, template syntax, configuration languages) are treated as **data** unless they cause execution in such an engine
- SQL injection that reads/writes data = data (no FEC) → #2 only
- SQL injection that invokes xp_cmdshell, or template injection that reaches a code-evaluation call = triggers FEC execution → #2 → #7
- Natural-language instructions to a model or agent are **not FEC**: prompt injection is #1 (see #1 boundary tests); only code or commands the agent then executes are FEC (#1 → #7)
...
- Natural-language instructions to a model or agent — prompt injection (direct or indirect), jailbreaks, RAG or context poisoning, agent tool abuse — are **#1**: the model's designed function acting on input it was designed to accept, no implementation flaw, no FEC (v2.6)
- If the agent then executes code or commands whose content the attacker controls, including code it generated at the attacker's instruction → **#1 → #7** (R-EXEC); a flaw in the model-serving or agent software → **#2/#3**; a subverted third-party model, dataset or tool → **#10** at the TAE. The agent is the attacker's capability vector, not the actor (Axiom IV)
...
- If exploitation yields security impact without FEC execution (e.g., authz bypass, SQLi data read/write) → **#2** only (data-vs-code boundary, see #7); document outcomes as Data Risk Events
...
- If the defective logic **is itself** a communication-path control (certificate validation, chain of trust, hostname matching, expiry/revocation, channel encryption, algorithm negotiation) → **#5**, not #2/#3 (R-SPECIFIC, channel)
...
- Domain-specific expression languages (SQL, LDAP, XPath, GraphQL, template syntax, configuration languages) are **data** unless they cause execution in a general-purpose engine; injection that only reads or writes data is **#2/#3 without #7**. Natural-language instructions to a model are not FEC (prompt injection is #1)
...
- If a physical-layer property of the substrate is the exploited generic vulnerability → **#8** (R-SPECIFIC, substrate); attacker proximity or possession is not the test
...
### R-SPECIFIC — Specific Generic Vulnerability over Residual (v2.6)
...
#### Capacity clause (#6) — Capacity Exhaustion vs Implementation Defect
- If **primary mechanism** is volume/intensity exhausting finite resources → **#6**
- If **primary mechanism** is implementation defect (crash, hang, degradation, algorithmic complexity) → **#2/#3**
...
#### Channel clause (#5) — Channel Control vs Code Flaw
- If the defective logic **is itself** a communication-path control → **#5**
- Covered controls: peer authenticity (certificate validation, chain of trust, hostname matching, expiry/revocation checking), channel encryption, algorithm negotiation
- If the defect is **incidental** to that control (e.g. memory corruption in a TLS parser) → **#2/#3** per R-ROLE
...
- The test is constitutive versus incidental, not server versus client
- Note: the channel clause classifies the **weakness**; R-MITM sequences the **path** (position acquisition vs action). They do not conflict
...
#### Substrate clause (#8) — Physical Property vs Implemented Logic
...
- If the physical layer is only the **readout channel** for a defect in implemented logic → classify by that defect (**#2/#3** per R-ROLE)
...
- The location of a flaw — hardware, firmware, silicon — never determines the cluster
...
- **#10** MUST be placed at the **Trust Acceptance Event (TAE)** — the moment the third-party trust link is honored and the trust artifact becomes authoritative inside the target domain
- **Subversion requirement (v2.6):** a #10 step requires that the trust artifact, or the third party issuing it, was subverted before acceptance; a defect in a legitimately supplied component is classified where it is exploited
- Falsifiability test: remove the attacker's subversion of the third party — not the third party itself. If the step still succeeds, it was never #10
...
5. **Specific Beats Residual (R-SPECIFIC)** — Capacity exhaustion → #6; a defect that *is* a communication-path control → #5; a physical-layer property of the substrate → #8; otherwise defects are #2/#3 and designed functions are #1
...
- "DDoS" is a consequence (LoA event); `#6 Flooding Attack` is the threat causing it (by volume) — OR `#2/#3` if the mechanism is an implementation defect (R-SPECIFIC, capacity)
...
### System Risk Event (SRE) — the central event (v2.5, re-centred in the v2.5.1 erratum; v2.6 wording)
...
- **The SRE the framework defines is System Compromise — loss of control.** The behaviour, privileges, data, or trust relationships of a system — or of a communication relationship it takes part in — are brought outside what their owner controls, in service of an attacker's objective. It is reached **only** through cluster steps (the Attack row). There is no second SRE type. The attacker need not hold anything inside the system: #6 takes capacity out of its owner's control, passive #5 interception takes a communication relationship out of its owners' control.
- **Successes only (v2.6).** A cluster assignment asserts that the step succeeded and recorded its SRE. An attempted or blocked step records no SRE and is **not a step of an attack path**: it is control-side evidence for the control that stopped it (Axiom V), recorded as an indicator, not as a step. Do not add blocked exploits, failed logins or filtered phishing to a path.
...
- **Attack Path:** `#6 + [DRE: A]`
- **Rationale:** Primary mechanism is volume/intensity vs finite capacity (R-SPECIFIC, capacity)

## Example 10: ReDoS (#2)
- **Scenario:** Crafted input causes catastrophic regex backtracking
- **Attack Path:** `#2 + [DRE: A]`
- **Rationale:** Although it looks like "flooding," primary mechanism is implementation defect (R-SPECIFIC, capacity)
...
- **Rationale:** The update was subverted upstream before the target accepted it; #10 placed where trust link is honored (TAE); execution is #7 (R-SUPPLY)

## Example 13: Federated Identity — Two Cases (#4 → #1 vs #10 → #4 → #1)
- **Scenario A (IdP not subverted):** Attacker uses stolen credentials at an external IdP; the org SP accepts the resulting assertion
- **Attack Path A:** `#4 → #1`
- **Rationale A:** Credential use at the IdP is #4 (R-CRED). The SP honouring an assertion from an IdP that was not subverted is the trust link working as designed — not a step, not #10. Feature abuse is #1
- **Scenario B (IdP subverted):** Attacker subverts the vendor IdP or its federation trust material (e.g. a stolen signing key) and mints assertions for users they are not
- **Attack Path B:** `#10 ||[auth][@Vendor(IdP)→@Org(SP)]|| → #4 → #1`
- **Rationale B:** The SP's acceptance of the subverted authority is the TAE (#10); each impersonating assertion presented through it remains #4; feature abuse is #1. The subversion of the IdP itself is classified with its own clusters inside the vendor's domain
...
❌ **Don't call a flaw in a legitimately supplied component #10**
- Wrong: "Log4Shell is #10 because Log4j is a third-party library"
- Right: "Log4Shell is #2 — the flaw is exploited where the server-role component runs. #10 requires that the artifact or its supplier was subverted before acceptance; where flawed code came from is location, not generic vulnerability"

❌ **Don't record blocked attempts as steps**
- Wrong: "#2 (blocked by WAF) → #9 → #4"
- Right: "#9 → #4" — the blocked exploit recorded no SRE; report it as an indicator for the WAF, not as a path step
...
2. **Apply the R-* rules:** Reference specific rules (R-ROLE, R-EXEC, R-SPECIFIC, etc.)
